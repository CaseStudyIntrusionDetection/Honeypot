...
class TestAddMetaTag(unittest.TestCase):
    ...
    def test_add_meta_tag(self):
        ...
        add_meta_tag(self.page_location, self.index_page, config)
        with open(os.path.join(self.main_page_path, 'index.html')) as main:
            main_page = main.read()
        soup = BeautifulSoup(main_page, 'html.parser')
        assert(soup.find("meta",
                         attrs={"name": "google-site-verification"}) and soup.find("meta",
                                                                                   attrs={"name": "msvalidate.01"}))

    def test_add_meta_tag_with_empty_tags(self):
        config = configparser.ConfigParser()
        config['WEB-TOOLS'] = dict(google='', bing='')
        assert add_meta_tag(self.page_location, self.index_page, config) is None
